...
func (m *ControllerManager) Initialize() error {
	err := controllers.SetupWithManager(m.CtrlMgr, &options.Options{ScoutWaitTimeoutSeconds: m.ScoutWaitTimeoutSeconds, ScoutInitialDelaySeconds: m.ScoutInitialDelaySeconds})
	if err != nil {
		return err
	}
