...
func (r *fakeResponder) Error(w http.ResponseWriter, req *http.Request, err error) {
	if r.called {
		r.t.Errorf("Error DefaultResponder called again!\nprevious error: %v\nnew error: %v", r.err, err)
	}
...
func TestServeHTTP(t *testing.T) {
	...
	for i, test := range tests {
		func() {
			...
			proxyHandler := NewUpgradeAwareHandler(backendURL, nil, false, test.upgradeRequired, proxyHooks{responder: responder})
			...
			if test.expectError != nil {
				if !responder.called {
					t.Errorf("%d: DefaultResponder was not invoked", i)
					return
				}
	...
	for k, tc := range testcases {
		for _, redirect := range []bool{false, true} {
			...
			func() { // Cleanup after each test case.
				...
				proxyHandler := NewUpgradeAwareHandler(serverURL, tc.ProxyTransport, false, false, proxyHooks{responder: &noErrorsAllowed{t: t}})
	...
	for _, intercept := range []bool{true, false} {
		for _, code := range []int{400, 500} {
			t.Run(fmt.Sprintf("intercept=%v,code=%v", intercept, code), func(t *testing.T) {
				...
				proxyHandler := NewUpgradeAwareHandler(backendServerURL, nil, false, false, proxyHooks{responder: &noErrorsAllowed{t: t}})
	...
	for _, intercept := range []bool{true, false} {
		for _, code := range []int{200, 300, 302, 307} {
			t.Run(fmt.Sprintf("intercept=%v,code=%v", intercept, code), func(t *testing.T) {
				...
				// Set up a proxy pointing to a specific path on the backend
				proxyHandler := NewUpgradeAwareHandler(backendServerURL, nil, false, false, proxyHooks{responder: &fakeResponder{t: t}})
				proxyHandler.InterceptRedirects = intercept
...
func TestDefaultProxyTransport(t *testing.T) {
	...
	for _, test := range tests {
		...
		h := NewUpgradeAwareHandler(locURL, nil, false, false, proxyHooks{})
...
func TestProxyRequestContentLengthAndTransferEncoding(t *testing.T) {
	...
	for k, item := range table {
		...
		proxyHandler := NewUpgradeAwareHandler(backendURL, nil, false, false, proxyHooks{responder: responder})
		proxyServer := httptest.NewServer(proxyHandler)
		defer proxyServer.Close()
...
func TestErrorPropagation(t *testing.T) {
	...
	proxyHandler := NewUpgradeAwareHandler(backendURL, &fakeRT{err: expectedErr}, true, false, proxyHooks{responder: responder})
	...
	if !strings.Contains(responder.err.Error(), expectedErr.Error()) {
		t.Fatalf("DefaultResponder got unexpected error: %v, expected the error to contain %q", responder.err.Error(), expectedErr.Error())
	}
}
