...
func isExceedParent(current, old *v1.ResourceQuota, parent *quotav1.CubeResourceQuota) bool {
	for _, rs := range quota.ResourceNames {
		...
		if isExceed(parentHard, parentUsed, changed) {
			clog.Info("overload, resource: %v, parent hard: %v, parent used: %v, changed: %v", rs, parentHard.String(), parentUsed.String(), changed.String())
			return true
		}
	}
...
func refreshUsedResource(current, old *v1.ResourceQuota, parent *quotav1.CubeResourceQuota) *quotav1.CubeResourceQuota {
	for _, rs := range quota.ResourceNames {
		pUsed := parent.Status.Used
		pHard := parent.Spec.Hard
